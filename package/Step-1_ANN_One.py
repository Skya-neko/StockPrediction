...
feature = datasetDf.iloc[:, ~datasetDf.columns.isin(['date', 'close'])]
target = np.array(datasetDf['close'])                         # 將df中的'close'欄位，存成numpy的array



# 劃分訓練集，測試集
# 因為shuffle = False所以不用random_state=10,
feature_train, feature_test, target_train, target_test = train_test_split(feature,
                                                                          target,
                                                                          test_size=0.25,
                                                                          shuffle=False)
predictedDays = int(math.ceil(0.25*len(target)))    # pridectedDays用模型預測出的數據天數 - (nPeriod-1)
...
def main(paramDict):
    ...
    sgd = SGD(learning_rate=paramDict['learning_rate'],
              decay=paramDict['decay'],
              momentum=paramDict['momentum'],
    ...
    try:
        ...
        outputFilePath = './data/'
        modelName = 'rmseBeyond5'
        
        if rmse < 10:
            # if not  os.path.exists(outputFilePath):
            #     os.mkdir(outputFilePath)
            modelName = 'keras_'+rmseStr+'.h5'
            # model.save(outputFilePath + modelName)
